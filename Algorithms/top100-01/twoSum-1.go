...
/*
思路：
1. 哈希查找：遍历数组的同时，将数值与其索引存入 Map。
2. 补数计算：对于每个元素 v，计算我们需要寻找的目标差值 complement = target - v。
3. 动态验证：如果 complement 已存在于 Map 中，说明找到了一对和为 target 的数。
4. 返回索引：返回 complement 的索引和当前索引。
*/
